...
# Don't pre-allocate all VRAM — TF defaults to grabbing 100% of GPU memory
# at startup, which makes the system unusable while prep runs. With this on,
# TF allocates only what it needs (CREPE-full uses ~300 MB, medium ~150 MB).
...
flags.DEFINE_enum(
    ...
    'CREPE model size. Smaller = faster on GPU and uses less VRAM. For '
    'bass f0 estimation (range ~30-200 Hz), "medium" matches "full" within '
    'octave-error margin and is ~3-4x faster. Recommended: "medium" '
    '(default) for batch prep, "full" only if you observe octave-jump '
    'artefacts in the f0 contour.')
...
def main(_):
  ...
  # Override CREPE's default model_capacity. DDSP's compute_f0 calls
  # crepe.predict() without specifying model_capacity, so it uses CREPE's
  # default ('full'). We patch crepe.predict via functools.partial so every
  # subsequent call uses the size the user picked. This is the cleanest
  # way to swap models without forking ddsp.spectral_ops.
  import crepe
  import functools
  _orig_crepe_predict = crepe.predict
  crepe.predict = functools.partial(
      _orig_crepe_predict, model_capacity=FLAGS.crepe_model_size)
  logging.info(
      'CREPE model_capacity = %s (override --crepe_model_size to change).',
      FLAGS.crepe_model_size)
